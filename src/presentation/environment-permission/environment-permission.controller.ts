...
export class EnvironmentPermissionController {
  ...
  @ApiResponse({ status: 201, description: 'Environment permission created successfully', type: EnvironmentPermissionResponseDto })
  @ApiResponse({ status: 409, description: 'Environment permission already exists' })
  async create(@Body() createDto: CreateEnvironmentPermissionDto): Promise<{ data: EnvironmentPermissionResponseDto }> {
    const permission = await this.environmentPermissionService.create(createDto);
  ...
  @ApiResponse({ status: 200, description: 'Environment permissions retrieved successfully', type: [EnvironmentPermissionResponseDto] })
  async findAll(@Query('includeDeleted') includeDeleted?: string): Promise<{ data: EnvironmentPermissionResponseDto[] }> {
    const permissions = await this.environmentPermissionService.findAll(includeDeleted === 'true');
  ...
  @ApiResponse({ status: 200, description: 'Environment permission retrieved successfully', type: EnvironmentPermissionResponseDto })
  @ApiResponse({ status: 404, description: 'Environment permission not found' })
  async findById(@Param('id') id: string): Promise<{ data: EnvironmentPermissionResponseDto }> {
  ...
  @ApiResponse({ status: 200, description: 'Environment permission updated successfully', type: EnvironmentPermissionResponseDto })
  @ApiResponse({ status: 404, description: 'Environment permission not found' })
  async update(@Param('id') id: string, @Body() updateDto: UpdateEnvironmentPermissionDto): Promise<{ data: EnvironmentPermissionResponseDto }> {
    const permission = await this.environmentPermissionService.update(id, updateDto);
    return { data: EnvironmentPermissionMapper.toResponse(permission) };
  }
  ...
  @ApiResponse({ status: 204, description: 'Environment permission deleted successfully' })
  @ApiResponse({ status: 404, description: 'Environment permission not found' })
  ...
  @ApiResponse({ status: 404, description: 'Environment permission not found' })
  async restore(@Param('id') id: string): Promise<{ data: EnvironmentPermissionResponseDto }> {
  ...
  @ApiResponse({ status: 204, description: 'Environment permission permanently deleted' })
  @ApiResponse({ status: 404, description: 'Environment permission not found' })
