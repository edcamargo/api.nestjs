...
export class LoggerService implements ILogger {
  ...
  private getSeverity(level: 'log' | 'error' | 'warn' | 'debug' | 'verbose'): SeverityNumber {
    const severityMap = {
  ...
  private emit(
    ...
    console.log(`${color}[${timestamp}] [${severityText}]${reset} ${contextStr} ${message}`);
  ...
  logRequest(method: string, url: string, statusCode: number, responseTime: number) {
    this.emit('log', `${method} ${url} ${statusCode} - ${responseTime}ms`, 'HTTP', {
      type: 'http_request',
      method,
      url,
      statusCode,
  ...
  logAuth(userId: string, email: string, action: 'login' | 'logout' | 'token_refresh') {
