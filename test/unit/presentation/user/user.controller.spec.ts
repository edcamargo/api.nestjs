  ...
  let mockService: Partial<Record<keyof UserService, jest.Mock>> & {
    ...
    describe('create', () => {
      it('should create a user and return response', async () => {
        const now = new Date();
        const user = new User('id-3', 'Alice', 'alice@example.com', 'hashed', UserRole.USER, now, now);
        mockService.create!.mockResolvedValue(user);

        const dto = { name: 'Alice', email: 'alice@example.com', password: 'secret', role: UserRole.USER } as any;
      ...
      it('should propagate ConflictException when create fails due to email in use', async () => {
        mockService.create!.mockRejectedValue(new ConflictException('Email already in use'));
        const dto = { name: 'Bob', email: 'bob@example.com', password: 'secret' } as any;
    ...
    describe('findById', () => {
      it('should return a user when found', async () => {
        const now = new Date();
        const user = new User('id-4', 'Carl', 'carl@example.com', 'hashed', UserRole.USER, now, now);
      ...
      it('should throw NotFoundException when user does not exist', async () => {
        mockService.findById!.mockRejectedValue(new NotFoundException('User not found'));
    ...
    describe('update', () => {
      it('should update and return updated user', async () => {
        const now = new Date();
        const user = new User('id-5', 'Dana', 'dana@example.com', 'hashed', UserRole.USER, now, now);
      ...
      it('should propagate NotFoundException when updating non-existent user', async () => {
        ...
        await expect(controller.update('nope', { name: 'x' } as any)).rejects.toThrow(NotFoundException);
      });

      it('should propagate ConflictException when email is already in use', async () => {
        mockService.update!.mockRejectedValue(new ConflictException('Email already in use'));
        await expect(controller.update('id', { email: 'taken@example.com' } as any)).rejects.toThrow(ConflictException);
      });
    });
    ...
    describe('hardDelete', () => {
      ...
      it('should throw NotFoundException when hard deleting non-existent user', async () => {
        mockService.hardDelete!.mockRejectedValue(new NotFoundException('User not found'));
        await expect(controller.hardDelete('missing')).rejects.toThrow(NotFoundException);
      });
    });
    ...
    describe('restore', () => {
      it('should restore and return user on success', async () => {
        const now = new Date();
        const user = new User('id-8', 'Eve', 'eve@example.com', 'hashed', UserRole.USER, now, now);
      ...
      it('should throw NotFoundException when restoring non-existent user', async () => {
        mockService.restore!.mockRejectedValue(new NotFoundException('User not found'));
        await expect(controller.restore('missing')).rejects.toThrow(NotFoundException);
      });

      it('should throw ConflictException when user is not deleted', async () => {
        mockService.restore!.mockRejectedValue(new ConflictException('User is not deleted'));
        await expect(controller.restore('not-deleted')).rejects.toThrow(ConflictException);
      });
    });
